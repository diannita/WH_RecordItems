<?php
    ...
    include_once ('../Configuration/config.php');
        ...
        $owner = $_POST['owner'];
        $dropdown_location = $_POST['location'];
        ...
        $query_list = "SELECT product.Code, product.ProductName, Product.AlphaCode FROM dbo.Owner LEFT JOIN Product ON (dbo.Owner.Code = Product.owner) WHERE dbo.Owner.Code =  $owner AND product.disabled = 0 ORDER BY ProductName ASC";
        $result = sqlsrv_query ($conn, $query_list);
        ...
        $cadena_product="<label class='text-gray-700 text-base'>Select Owner Product</label>        
            ...
            while ($data = sqlsrv_fetch_array($result, SQLSRV_FETCH_ASSOC)) {
                    ...
                    '<option value='.$data['Code'].'>'.utf8_encode($data['ProductName'].' -- '.$data['AlphaCode']).'</option>';
            }
            echo  $cadena_product.
            "</select> <br>";
